...
let fitToViewRef = null  // 当前实例的缩放适配函数（renderGraph 内赋值）
...
const renderGraph = () => {
  ...
  const node = nodeGroup.selectAll('circle')
    ...
    .on('click', (event, d) => {
      ...
      linkGroup.selectAll('path').attr('stroke', '#C0C0C0').attr('stroke-width', 1.5)
      // 高亮选中节点
      d3.select(event.target).attr('stroke', '#FF4500').attr('stroke-width', 4)
      // 高亮与此节点相连的边
  ...
  // 自动适配视口：仿真早期先收一次（快速回中），结束后再收一次（最终居中）
  let autoTick = 0
  let earlyFitted = false
  simulation.on('tick.autofit', () => {
    autoTick++
    if (!earlyFitted && autoTick >= 80) {
      earlyFitted = true
      fitToViewFn()
    }
  })
  simulation.on('end.autofit', fitToViewFn)
...
}, { deep: true, immediate: true })  // immediate：组件重建（:key 变化）时数据已就位，需立即渲染
